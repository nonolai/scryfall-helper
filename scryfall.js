...
function getSuggestions(query, maxResults = 5) {
  ...
  if (!lastToken.includes(':')) {
    return new SuggestionData(atoms.find(lastToken.toLowerCase()).slice(0, maxResults), query.length - lastToken.length);
  ...
  return new SuggestionData(possibleValues.find(partialValue.toLowerCase()).slice(0, maxResults), splitterIdx);
}
...
const vm = new Vue({
  ...
  methods: {
    takeSuggestion: function () {
      if (this.suggestions && this.suggestions.length > this.currentSuggestion) {
        const presplitter = this.query.substring(0, this.lastSplitterIdx);
        this.query = presplitter + this.suggestions[this.currentSuggestion];
      }
    },
    submit: function() {
      const uri = "https://scryfall.com/search?q=" + encodeURI(this.query);
      location.href = uri;
    },
